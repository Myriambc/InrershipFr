...
function LevelOrder(props) {
  const classes = useStyles();
  const dispatch = useDispatch();

  useEffect(() => {
    if (levelsGrouped?.length === 0) {
      dispatch(getGroupedLevels("?grouped=true&sort=-order,title"));
    }
  }, [dispatch]);

  const { levelsGrouped, loading } = useSelector((state) => state.levels);
  ...
  useEffect(() => {
    buildList(levelsGrouped, setTitleList, setList);
  }, [dispatch, levelsGrouped]);

  const updateOrders = async (index) => {
    const orders = filterList(index, list);
    await dispatch(updateLevelOrder(orders)).then((response) => {
  ...
  return (
    <Grid container spacing={3} className={classes.root}>
      <Grid item xs={12}>
        <SimpleToolbar title={"Reorder Levels"} />
      </Grid>
      {loading === "loading" ? (
        <h2>loading...</h2>
      ) : (

...
function ExerciseOrder() {
  ...
  const { groupedExercises, loading } = useSelector((state) => state.exercices);
  ...
  useEffect(() => {
    buildList(groupedExercises, setTitleList, setList);
  }, [dispatch, groupedExercises]);
  ...
  const updateOrders = async (index) => {
    ...
    await dispatch(updateExerciseOrder(orders)).then((response) => {
  ...
  return (
    <Grid container spacing={3} className={classes.root}>
      <Grid item xs={12}>
        <SimpleToolbar title={"Reorder Exercises"} />
      </Grid>
      {loading === "loading" ? (
        <h2>loading...</h2>
      ) : (

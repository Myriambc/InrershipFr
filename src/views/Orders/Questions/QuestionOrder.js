...
function ExerciseOrder() {
  ...
  const { groupedQuestions, loading } = useSelector((state) => state.questions);
  ...
  useEffect(() => {
    buildList(groupedQuestions, setTitleList, setList);
  }, [dispatch, groupedQuestions]);
  ...
  const updateOrders = async (index) => {
    ...
    await dispatch(updateQuestionsOrder(orders)).then((response) => {
  ...
  return (
    <Grid container spacing={3} className={classes.root}>
      <Grid item xs={12}>
        <SimpleToolbar title={"Reorder Questions"} />
      </Grid>
      {loading === "loading" ? (
        <h2>loading...</h2>
      ) : (
